...
- **作者没有在真实游戏里跑过这个界面**：构建环境里没有安装 Minecraft 本体，所以「编译通过 + 逻辑测试通过」是验证到的程度，界面交互请用 `runClient` 过一遍（见下面的验证情况）。
...
- **71 个单元测试全绿**（38 个语句生成与速率换算 + 9 个图谱持久化 + 18 个 SFM 真实编译器验证 + 6 个画布坐标），其中那 18 个会把生成的程序交给 **SFM 自己的 ANTLR 解析器 + AST 构建器**验证——这一步不是形式主义，它抓出了两个真实缺陷：
